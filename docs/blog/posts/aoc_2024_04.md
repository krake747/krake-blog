...
=== "F#"
    ...
    module Fay04 =
        ...
        let wordSearchXmas (map: Map<Point, char>) =
            ...
            let wordSearchMapForXmas = wordSearch map "XMAS"

            map.Keys
            |> Seq.collect (fun p -> directions |> Seq.map (wordSearchMapForXmas p))
            |> Seq.filter id

        let wordSearchXmasShape (map: Map<Point, char>) =
            let wordSearchMapForXmasShape = wordSearch map "MAS"

            map.Keys
            |> Seq.filter (fun p ->
                wordSearchMapForXmasShape (p + Point.NorthWest) Point.SouthEast
                && wordSearchMapForXmasShape (p + Point.SouthWest) Point.NorthEast)

        let part1 (input: AocInput) =
            input.AllLines |> parseMap |> wordSearchXmas |> Seq.length

        let part2 (input: AocInput) =
            input.AllLines |> parseMap |> wordSearchXmasShape |> Seq.length
    ```
...
=== "Python"
    ...
    def parseMap(lines: list[str]) -> Map:
        return defaultdict(str, {Point(x, y): c for y, l in enumerate(lines) for x, c in enumerate(l)})


    def word_search(map: Map, pattern: str, p: Point, dir: Point) -> bool:
        chars = [map.get(p + dir * i) for i in range(0, len(pattern))]
        return chars == list(pattern) or chars == list(reversed(pattern))


    def word_search_xmas(map: Map) -> list[bool]:
        directions = [Point.East(), Point.SouthEast(), Point.South(), Point.SouthWest()]
        word_search_map_for_xmas = partial(word_search, map, "XMAS")
        return [word_search_map_for_xmas(p, dir) for p in map.keys() for dir in directions]


    def word_search_large_xmas(map: Map) -> list[bool]:
        word_search_map_for_large_xmas = partial(word_search, map, "MAS")
        return [
            word_search_map_for_large_xmas(p + Point.NorthWest(), Point.SouthEast())
            and word_search_map_for_large_xmas(p + Point.SouthWest(), Point.NorthEast())
            for p in map.keys()
        ]


    def part1(input: AocInput) -> int:
        return sum(word_search_xmas(parseMap(input.lines)))


    def part2(input: AocInput) -> int:
        return sum(word_search_large_xmas(parseMap(input.lines)))
    ```
